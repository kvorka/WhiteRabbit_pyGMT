submodule (harmsy) harmsy4
  ...
  module pure subroutine harmsy4_sub(jmax, spectra, gridvals)
    ...
    integer                         :: it, ip, ij, im, ijm, imj, iv, i1
    ...
    it = 0
      ...
      im = 0
        ...
        ij = im
          do concurrent ( iv = 1:4 )
            sumLege(iv,1,0) = spectra(iv,1) * pmj(1)
          end do
        ...
        do ij = 1, jmax
          ...
          do concurrent ( iv = 1:4 )
            sumLege(iv,1,0) = sumLege(iv,1,0) + spectra(iv,ijm) * pmj(1)
          end do
        end do
      ...
      do ip = 1, 2*nth
        !im = 0
          do concurrent ( iv = 1:4 )
            gridvals(iv,ip,it) = sumLege(iv,1,0)%re
          end do
      end do
    ...
    do it = 1, nth, 4
      ...
      im = 0
        ...
        ij = im
          do concurrent ( i1 = 1:4, iv = 1:4 )
            sumLege(iv,i1,0) = spectra(iv,1) * pmj(i1)
          end do
        ...
        do ij = 1, jmax
          ...
          do concurrent ( i1 = 1:4, iv = 1:4 )
            sumLege(iv,i1,0) = sumLege(iv,i1,0) + spectra(iv,ijm) * pmj(i1)
          end do
        end do
      ...
      do im = 1, jmax
        ...
        ij = im
          ...
          do concurrent ( i1 = 1:4, iv = 1:4 )
            sumLege(iv,i1,im) = sumLege(iv,i1,im) + spectra(iv,ijm) * pmj(i1)
          end do
      ...
      do ip = 1, 2*nth
        im = 0
          ...
          do concurrent ( i1=1:4, iv = 1:4 )
            gridvals(iv,ip,it+i1-1) = sumLege(iv,i1,0)%re
          end do
        ...
        do im = 1, jmax
          ...
          do concurrent ( i1 = 1:4, iv = 1:4 )
            gridvals(iv,ip,it+i1-1) = gridvals(iv,ip,it+i1-1) + 2 * real( expmul * sumLege(iv,i1,im) , kind=dbl )
          end do
        end do
      end do
    end do
