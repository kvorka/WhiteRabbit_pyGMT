...
class plot:
  ...
  def __init__(self,mode=''): 
    ...
    self.subplot=0    # a variable to track multiple subplots if present
    ...
    self.picname=''
    #self.put("line loctype world;box loctype world; string loctype world")
    # this stores all the gracecommands...
    self.gracedict={
    ...
    "sts"          :[  "subtitle size "                ,   ""   ,0],
    "xa"           :[  "xaxis  "                       ,   ""   ,0], # on off
    "xb"           :[  "xaxis bar  "                   ,   ""   ,0], # on off
    "xac"          :[  "xaxis bar color  "             ,   ""   ,0], 
    "xtz"          :[  "xaxis type zero  "             ,   ""   ,0], # true false
    "ax"           :[  "autoscale xaxes  "             ,   ""   ,0], 
    "xl"           :[  "xaxis  label "                 ,   ""   ,1],
    "xls"          :[  "xaxis  label char size "       ,   "2"  ,0],
    "xlf"          :[  "xaxis  label font "            ,   ""   ,0],
    "xlp"          :[  "xaxis  label place "           ,   ""   ,0], # both normal opposite
    "xtoo"         :[  "xaxis  tick       "            ,   ""   ,0], # on off
    "xt"           :[  "xaxis  tick major "            ,   ""   ,0],
    "xtg"          :[  "xaxis  tick major grid "       ,   ""   ,0], # on off
    "xts"          :[  "xaxis  tick major size "       ,   ""   ,0],
    "xtlw"         :[  "xaxis  tick major linewidth "  ,   ""   ,0],
    "xtlst"        :[  "xaxis  tick major linestyle "  ,   ""   ,0],
    "xtc"          :[  "xaxis  tick major color "      ,   ""   ,0],
    "xtm"          :[  "xaxis  tick minor ticks "      ,   ""   ,0],
    "xtmg"         :[  "xaxis  tick minor grid "       ,   ""   ,0], # on off
    "xtms"         :[  "xaxis  tick minor size "       ,   ""   ,0],
    "xtmlw"        :[  "xaxis  tick minor linewidth "  ,   ""   ,0],
    "xtmlst"       :[  "xaxis  tick minor linestyle "  ,   ""   ,0],
    "xtmc"         :[  "xaxis  tick minor color "      ,   ""   ,0],
    ...
    "xbl"          :[  "xaxis  bar linewidth "         ,   ""   ,0], # both normal opposite
    "ya"           :[  "yaxis  "                       ,   ""   ,0], # on off
    "yb"           :[  "yaxis bar  "                   ,   ""   ,0], # on off
    "ytz"          :[  "yaxis type zero  "             ,   ""   ,0], # true false
    "yac"          :[  "yaxis color  "                 ,   ""   ,0], 
    "ay"           :[  "autoscale yaxes  "             ,   ""   ,0], 
    "yl"           :[  "yaxis  label "                 ,   ""   ,1],
    "yls"          :[  "yaxis  label char size "       ,   "2"  ,0],
    "ylf"          :[  "yaxis  label font "            ,   ""   ,0],
    "ylp"          :[  "yaxis  label place "           ,   ""   ,0], # both normal opposite
    "ytoo"         :[  "yaxis  tick       "            ,   ""   ,0], # on off
    "yt"           :[  "yaxis  tick major "            ,   ""   ,0],
    "ytg"          :[  "yaxis  tick major grid "       ,   ""   ,0], # on off
    "yts"          :[  "yaxis  tick major size "       ,   ""   ,0],
    "ytlw"         :[  "yaxis  tick major linewidth "  ,   ""   ,0],
    "ytlst"        :[  "yaxis  tick major linestyle "  ,   ""   ,0],
    "ytc"          :[  "yaxis  tick major color "      ,   ""   ,0],
    "ytm"          :[  "yaxis  tick minor ticks "      ,   ""   ,0],
    "ytmg"         :[  "yaxis  tick minor grid "       ,   ""   ,0], # on off
    "ytms"         :[  "yaxis  tick minor size "       ,   ""   ,0],
    "ytmlw"        :[  "yaxis  tick minor linewidth "  ,   ""   ,0],
    "ytmlst"       :[  "yaxis  tick minor linestyle "  ,   ""   ,0],
    "ytmc"         :[  "yaxis  tick minor color "      ,   ""   ,0],
    ...
    "strf"         :[  "string font "                  ,   ""   ,0],  
    "strj"         :[  "string just "                  ,   ""   ,0],  
    "strcs"        :[  "string char size "             ,   ""   ,0],  
    "strlt"        :[  "string loctype "               ,   ""   ,0], # view or world 
    "box"          :[  "with box \
                        \n  box on \
                        \n  box g0 \
                        \n  box def \
                        \n  box"                      ,   ""   ,0], # user drawn line (ie. NOT data)
    "boxc"         :[  "box color        "            ,   ""   ,0],  
    "boxlt"        :[  "box loctype      "            ,   ""   ,0], # view or world 
    "boxlw"        :[  "box linewidth    "            ,   ""   ,0],  
    "boxls"        :[  "box linestyle    "            ,   ""   ,0],  
    "boxfc"        :[  "box fill color   "            ,   ""   ,0],  
    "boxfp"        :[  "box fill pattern "            ,   ""   ,0],  
    "linel"        :[  "with line \
                        \n  line on \
                        \n  line g0 \
                        \n  line def \
    ...
    "linec"        :[  "line color        "            ,   ""   ,0],  
    "linelt"       :[  "line loctype      "            ,   ""   ,0], # view or world 
  ...
  def data(self,dlist,space_delimited='',pcol=[],use_comments=False):
    ...
    for ii,i in enumerate(dlist):
      ...
      if use_comments:
        commands+=os.popen("grep '#gy ' %s"%i).read().replace("#gy ","")
    ...
    if commands:self.quickin(commands)
  ...
  def function(self,inp1,inp2=[]):
    ...
    if not len(inp2):
      for i in inp1:
        for j in i:print(j, end=' ', file=OUT)
        print("", file=OUT)
      OUT.close()
    else:
      if len(inp1)!=len(inp2):print("improper input arrays: lengths differ.");return
      for i,ii in enumerate(inp1):
        print(ii, end=' ', file=OUT)
        if not dir(inp2[i]).count("__iter__"):print(inp2[i], end=' ', file=OUT)
        else:
          for j in inp2[i]:print(j, end=' ', file=OUT)
        print("", file=OUT)
      OUT.close()
    self.data('%s/.ppfuncfile'%self.ddir+ext);self.funcs+=1

  def defaults(self,file=''):
    "Apply default values from the supplied file."
    if not file:
      if os.path.exists("%s/.gracey_defaults"%self.ddir):
        self.quickin(open("%s/.gracey_defaults"%self.ddir).read())
  ...
  def show(self,device=''):
    ...
    # now output the batch file...
    OUT=open("%s/.ppbatchfile"%self.ddir,'w'); print(self.batch, file=OUT)
    # actually, i have gone back to using the put method... just makes more sense...
#    # write the results of the gracedict to the batchfile... do not use put method...
#    header='';gdict=self.gracedict
#    for i in gdict.keys():
#      if gdict[i][1]:
#        if type(gdict[i][1])==type(''):header+='%s %s\n'%(gdict[i][0],gdict[i][1]) 
#        else:
#          for j,jj in enumerate(gdict[i][1]):header+='s%s %s %s\n'%(j,gdict[i][0],jj)
#    if header:OUT.write(header)
    ...
    if not device and self.mode!="interactive": 
      ...
      print("xmgrace -noask -nosafe -batch %s/.ppbatchfile"%self.ddir)
    elif self.devlist.count(device):
      if not device=='null':
        os.system("xmgrace -nosafe -noask -hardcopy -hdevice %s -batch %s/.ppbatchfile"%(device,self.ddir))
        sys.stderr.write("xmgrace -nosafe -noask -hardcopy -hdevice %s -batch %s/.ppbatchfile\n"%(device,self.ddir))
        if self.picname:
          # created directory if it does not exist...
          if self.picname.count("/"): 
            temp="/".join(self.picname.split("/")[:-1])
            if not os.path.exists(temp):
              sys.stderr.write("Directory %s does not exist, creating...\n"%temp)
              try:    os.mkdir(temp)
              except: sys.stderr.write("Could not create %s...\n"%temp);sys.exit()
          # now move file...
          os.system("mv Untitled.%s %s.%s"%(self.devdict[device],self.picname,self.devdict[device]))
      else:
        # for some reason "saveall" was not working at the command line...
        OUT=open("%s/.ppbatchfile"%self.ddir,'a')
        OUT.write('saveall "%s/.ppgraceinput" \n'%self.ddir)
        OUT.close()
        os.system("xmgrace -nosafe -noask -hardcopy -batch %s/.ppbatchfile "%self.ddir)
        sys.stderr.write("xmgrace -nosafe -noask -hardcopy -batch %s/.ppbatchfile \n"%self.ddir)
  ...
  def gracefunc(self,inp):
    ...
    """
    if inp.count('help'): print("""selected grace functions. see manual for more details. \
    \nabs(x) \nacos(x) \nacosh(x) \nasin(x) \nasinh(x) \natan(x) \natanh(x) \
    \nceil(x) \ncos(x) \ncosh(x) \nexp(x) \nfac(n) \nfloor(x) \nirand(n) \nln(x) \
    \nlog10(x) \nlog2(x) \nmod(x,y) \npi \nrand \nrint(x) \nrsum(x) \nsin(x) \
    \nsinh(x) \nsqr(x) \nsqrt(x) \ntan(x) \ntanh(x) \nerf(x) \nerfc(x) \nfdtrc(x) \
  ...
  def operate_on_set(self,input):
    ...
    for inp in input:
      sss=inp[0]
      if not sss=='x' and not sss=='y':print("must give x or y as first character in string");self.error+=1;return
      ...
      if len(setnum)==2:setnum=list(range(setnum[0],setnum[1]+1))
      elif len(setnum)==0:setnum=list(range(sum(self.fileset)))
      ...
      for i in setnum:
        ...
        self.put("S%s.%s=S%s.%s%s"%(str(i),sss,str(i),sss,oper))
  ...
  def fat_contour(self,files):
    """this method is similar to contour, except instead of using color it uses the size of the point from the third column. You should scale
       your results however you need to get reasonable point sizes."""
    # create a color map...
    if type(files)!=type([]):files=[files]
    for i in files:
      # find number of sets in file
  ...
  def oldcandles(self,inp,bw=1.,sw=0.1):
    """this method is used to make candles for stock plots. the input is:
       x,open,high,low,close """
    # find max/min
    temp=[];buf=.15;xr=inp[-1][0]-inp[0][0]
    for i in inp:temp+=i[1:]
    temp.sort();yr=temp[-1]-temp[0]
    self.function([inp[0][0]-xr*buf,inp[-1][0]+xr*buf],[temp[0]-yr*buf,temp[-1]+yr*buf])
    self.quickin("xmin=%.6f xmax=%.6f ymin=%.6f ymax=%.6f"%(inp[0][0]-xr*buf,inp[-1][0]+xr*buf,temp[0]-yr*buf,temp[-1]+yr*buf))
    bw=xr/(len(inp)*1.2)
    sw=bw/20.

    temp="box loctype world\n"
    for i,ii in enumerate(inp):
      # determine color...
      if i>0:
        if ii[4]<inp[i-1][4]:temp+="box color 2;box fill color 2\n"
        else:temp+="box color 1;box fill color 1\n"
      # do body first... Open|High|Low|Close...
      if ii[4]-ii[1]<0:temp+="box fill pattern 1\n"
      else:temp+="box fill pattern 0\n"
      temp+="with box\n  box on\n  box def\n  box %.4f,%.4f,%.4f,%.4f\n"%(ii[0]-bw/2.,ii[1],ii[0]+bw/2.,ii[4])
      # now do stem...
      temp+="with box\n  box on\n  box def\n  box %.4f,%.4f,%.4f,%.4f\n"%(ii[0]-sw/2.,ii[2],ii[0]+sw/2.,ii[3])
    self.put(temp)

  def candles(self,inp):
    """this method is used to make candles for stock plots. the input is:
       x,open,high,low,close """
    # create file with properly arranged data... 
    # file has Open|High|Low|Close...
    # xmgrace has line|open|close|low|high...
    inp=[[str(s)]+ss.split()[1:] for s,ss in enumerate(open(inp))]
    up=[ss for s,ss in enumerate(inp[1:]) if ss[4]>=inp[s][4]]
    up.append(inp[0])
    dn=[ss for s,ss in enumerate(inp[1:]) if ss[4]<inp[s][4]]
    hollowup=[s for s in up if float(s[4])>=float(s[1])]
    hollowdn=[s for s in dn if float(s[4])>=float(s[1])]
    solidup=[s for s in up if float(s[4])<float(s[1])]
    soliddn=[s for s in dn if float(s[4])<float(s[1])]
    # if one type of candle is not present, just fib it so quickin is not messed up...
    if not hollowup:hollowup=[['1 %s %s %s %s'%(inp[0],inp[0],inp[0],inp[0])]]
    if not hollowdn:hollowdn=[['1 %s %s %s %s'%(inp[0],inp[0],inp[0],inp[0])]]
    if not solidup:solidup=[['1 %s %s %s %s'%(inp[0],inp[0],inp[0],inp[0])]]
    if not soliddn:soliddn=[['1 %s %s %s %s'%(inp[0],inp[0],inp[0],inp[0])]]
    # reorder things...
    hollowup="\n".join([" ".join([s[0],s[1],s[1],s[4],s[3],s[2]])  for s in hollowup ])
    hollowdn="\n".join([" ".join([s[0],s[1],s[1],s[4],s[3],s[2]])  for s in hollowdn ])
    solidup="\n".join([" ".join([s[0],s[1],s[1],s[4],s[3],s[2]])  for s in solidup ])
    soliddn="\n".join([" ".join([s[0],s[1],s[1],s[4],s[3],s[2]])  for s in soliddn ])
    #OUT=open("candledata",'w');OUT.write(inp+'\n');OUT.close()
    OUT=open("candledata",'w')
    OUT.write('#hollowup\n'+hollowup+'\n\n#hollowdn\n'+hollowdn+'\n\n#solidup\n'+solidup+'\n\n#soliddn\n'+soliddn+'\n')
    OUT.close()
    self.fileset.append(4)
    self.quickin("l=0 sfp=[0,0,1,1] sfc=[bl,re,bl,re] sc=[bl,re,bl,re] ebs=0 ebc=1",use_default=False)
    self.put('read xyboxplot "candledata"')

  def get_limits(self,file=''):
    """ This method determines retrieves xmgrace's values for the bounds of the
    plot (ie. xmin, xmax, ymin, ymax). This is useful when you want to use
    these bounds for defining lines or string positions but dont want to have
    to always specify them.  This is kind of clunky as we actually have to fire
    up xmgrace and then grep the values out of the output, but it works.
    """
    if not file:
      self.show('null')
      temp=os.popen('grep "@    world [0-9-]" %s/.ppgraceinput'%self.ddir).readline().split("world")[-1].strip()
    else: 
      if os.path.isfile(file):
        temp=os.popen('grep "@    world [0-9-]" %s'%file).readline().split("world")[-1].strip()
      else:print("Cannot get xmin/xmax/ymin/ymax from %s: file does not exist."%file);sys.exit()
    self.gracedict['w'][1]=temp

  def quickin(self,modparam,nset=0,use_default=True):
    ...
    item the default value (deprecated, now use defaults method) of the grace
    command. in the case of commands corresonding to data set quantities (ie.
    line color), the value of the grace command is given as a list. this method
    takes in a string such as "t=plot1 xl=eV" and will look through the
    dictionary to find the proper grace command. any text will automatically be
    quoted. set quantities are specified as "s=[1,2,3]", which correspond to a
    plot with three sets, and assigns symbol type one for set one, etc.  one
    may also specify plot defaults by manually modifying the dictionary
    below."""

    gdict=self.gracedict.copy() # define the dictionary with all the grace commands...
    if not nset:nset=sum(self.fileset)
    if type(modparam)!=type([]):modparam=modparam.split()
    if [s for s in modparam if not s.count("=")]:print("all quickin entries must have =");sys.exit()
    # convert to list of list by splitting on first =
    modparam=[[s.split("=")[0],"=".join(s.split("=")[1:])] for s in modparam]
    # ensure that limits (ie. w=, xmin=, etc) are at beginning...
    temp1=[];temp2=[]
    for j,jj in enumerate(modparam):
      if ['w','xmin','xmax','ymin','ymax'].count(jj[0]):temp1.append(jj)
      else:temp2.append(jj)
    modparam=temp1+temp2
    ...
    for i in [s for s in modparam if s[0]=="str"]:
    ...
    # this allows for xr=a,b and yr=a,b to specify ranges...
    for ii in [s for s in modparam if s[0]=="xr" or s[0]=="yr"]:
      tt=ii[-1].split(",");del modparam[modparam.index(ii)]
      if ii[0]=="xr":   modparam=[["xmin",tt[0]],["xmax",tt[1]]]+modparam
      elif ii[0]=="yr": modparam=[["ymin",tt[0]],["ymax",tt[1]]]+modparam

    # grace format is w=xmin,ymin,xmax,ymax. awkward for me. change to w=xmin,xmax,ymin,ymax...
    tt=[s for s,ss in enumerate(modparam) if ss[0]=="w"]
    if tt: 
      temp=modparam[tt[0]][1].split(",")
      modparam[tt[0]][1]="%s,%s,%s,%s"%(temp[0],temp[2],temp[1],temp[3])
      
    #################################

    # check to be sure all items are in dictionary...
    bad=[s[0] for s in modparam if not list(gdict.keys()).count(s[0])]
    if bad:
      for i in sorted(gdict.keys()):
        print("    %s%s<-->    %s%s  default=%s"%(i," "*(10-len(i)),gdict[i][0]," "*(30-len(gdict[i][0])),gdict[i][1]))
      for i in bad: print("there is no quickin keyword '%s'. see above printout for proper shortcut.\n"%i)
      sys.exit()
    
    # this block of code updates the grace dictionary...
    for i in modparam: 
      ...
      if type(gdict[i[0]][1]) is type([]) and i[1].count("["):
        ...
        if len(i[1])!=nset: print("improper array size given for input variable %s"%i[0]);sys.exit()
      kw=i[0]; gval=i[1]; quo=gdict[kw][2]  # rename these for easier syntax below...
      # now let us update the gracedict...
      if type(gval) is not type([]): # scalars first...
        # see if we need to replace any shortcuts for colors, symbols, etc...
        if list(self.styledict.keys()).count(gval):gval=self.styledict[gval]
        # add quotes if needed... and sub for white space...
        if quo: gval='"%s"'%gval.replace("__"," ").replace(".sp."," ")
#        # repetetive commands (ie. str line box) need to be "put" immediately, not stored in gdcit...
#        if sum([kw.count(s) for s in ["str",'line','box']]):
        # check to see if we need to sub in xmin, etc...
        if kw=='strzl' or kw=='linel':
          for j,jj in enumerate(['xmin','ymin','xmax','ymax']):
            if gval.count(jj):
              if gdict[jj][1]:gval=gval.replace(jj,gdict[jj][1])
              elif gdict['w'][1]: gval=gval.replace(jj,gdict['w'][1].split(",")[j])
              else:print("cannot sub %s in %s=%s as it has not been defined..."%(jj,i[0],i[1]));sys.exit()
          # eval expression to allow for arithmetic...
          gval=",".join([str(eval(s)) for s in gval.split(",")])  
        # store the parameter...      
        gdict[kw][1]=gval
        # deposit the parameter... replace is for str, line, and box in case of multiple subplots...    
        # it will not effect anything else (see gracedict). a bit hacky but it is sound.
        self.put('%s %s'%(gdict[kw][0].replace("g0","g%s"%self.subplot),gval))
      else: # now lists...
        # see if we need to replace any shortcuts for colors, symbols, etc...
        gval=[self.styledict[s] if list(self.styledict.keys()).count(s) else s for s in gval]
        # add quotes if needed... and sub for white space...
        if quo:gval=['"%s"'%s.replace("__"," ").replace(".sp."," ") for s in gval]
        # update value...
        gdict[kw][1]=[s for s in gval]
        # deposit the results...
        for j,jj in enumerate(gdict[kw][1]):self.put('s%s %s %s'%(j,gdict[kw][0],jj))




  def oldquickin(self,modparam,nset=0,use_default=True,debug=''):
    """this method allows most grace commands with a shortcut syntax. a
    dictionary is used to store the shortcut names as keys, and the values are
    a two item list with the first item being the grace command and the second
    ...
    if use_default: default=[[s,gdict[s][1]] for s in list(gdict.keys()) if gdict[s][1] and not [k[0] for k in modparam].count(s)]
    else:default=[]
    ...
    for i in default+modparam: 
      ...
      if type(gval) is not type([]): 
        ...
        for j in range(len(gval)): 
          # see if we need to replace any shortcuts for colors, symbols, etc...
          if list(self.styledict.keys()).count(gval[j]):gval[j]=self.styledict[gval[j]]
          # see if quotes are needed...
          if not quo:
...
if __name__=="__main__":
  ...
  ifile=[sys.argv[s+1] for s,ss in enumerate(sys.argv) if ss=="-i" and os.path.exists(sys.argv[s+1])]
  for i in ifile:
  ...
  options=[s for s in sys.argv if ['-fs','-cg','-cgfs','-s','-u','-op','-save','-con','-com','-coml','-i','-as','-gl','-fat'].count(s)]
  # check for plotting specific columns...
  #pcol=[sys.argv[s+1] for s,ss in enumerate(sys.argv) if ss.count("-u") and sys.argv[s+1].count(":")]
  pcol=[sys.argv[s+1] for s,ss in enumerate(sys.argv) if ss=="-u" and sys.argv[s+1].count(":")]
  ...
  if sys.argv.count("-con") or sys.argv.count("-fat"):
    try: confile=[sys.argv[s+1] for s,ss in enumerate(sys.argv) if (ss.count("-con") or ss.count("-fat")) and  
  ...
  if file.count('pipe') or confile.count('pipe'):
    ...
    for i in sys.stdin: print(i.strip(), file=OUT)
    OUT.close() 
  ...
  # grab grace functions from a comment in file...
  if options.count("-com"):  
    for f in file:
      if os.path.exists(f):
        temp=os.popen("grep '#gf ' %s"%f).read().replace("#gf ","")
        if temp:
          gracefunc.append("@"+temp)

  # add any user input grace functions...
  if gracefunc:mp.gracefunc([s[1:] for s in gracefunc])

  # now we can add the data to the plot...  
  if options.count("-com"):  use_comments=True
  else:                      use_comments=False
  if options.count("-s"):      mp.data(file,space_delimited=True,use_comments=use_comments) 
  elif options.count("-u"):    mp.data(file,pcol=pcol, use_comments=use_comments) 
  else:                        mp.data(file,use_comments=use_comments) 
  if options.count("-con"):    mp.contour(confile) 
  if options.count("-fat"):    mp.fat_contour(confile) 
  ...
  # perform an autoscale to previously saved file...
  if options.count("-as"): mp.get_limits("%s/.ppgraceinput"%mp.ddir)

  # defines plot range as determined by xmgrace...
  if options.count("-gl"): mp.get_limits()
  ...
  if options.count("-coml"):mp.comment_to_legend()
  ...
  # overide for now... no reason not to save...
  #if options.count("-save"):mp.save()
  # cannot save if you want a hard copy...
  if not hardcopy[0]:mp.save()

  # print out the resutling batch file that xmgrace will load and show plot...
  if not mp.error: mp.show(hardcopy[0]) #; print mp.batch 
